    ...
    : ICommandHandler<Command>
{
    ...
    {
        logger.LogInformation("Attempting to delete order product for order {OrderId} and product {ProductId}",
        ...
        if (item is null)
        {
            logger.LogWarning("Order product not found for order {OrderId} and product {ProductId}", request.OrderId,
                request.ProductId);
        ...
        logger.LogInformation("Order product removed from order {OrderId} and product {ProductId}", request.OrderId,
            request.ProductId);
